...
class MainAppFrame(tk.Frame):
    def __init__(self, master, user_email):
        ...
        tk.Button(action_frame, text="View Order History", command=self.view_orders).pack(side="left", padx=5)
    ...
    def view_orders(self):
        orders = self.user_profile.view_order_history()

        if not orders:
            messagebox.showinfo("Orders", "No past orders found.")
            return

        text = ""
        for o in orders:
            text += f"{o['order_id']} | {o['status']} | ${o['total']} | {o['date']}\n"

        messagebox.showinfo("Order History", text)
